      ...
      <div className="addTodo my-5">
        ...
        <div className="todos">
          ...
          return <div key={item.id} className="todo flex w-1/4 my-3 justify-between">
            ...
            <input name={item.id} onChange={handleChechbox} type="checkbox" value={item.isCompleted} id="" />
          <div className={item.isCompleted?"line-through":""}>{item.todo}</div>
            <div className="buttons">
              ...
              <button onClick={handleDelete} className='bg-violet-800 hover:bg-violet-950 p-2 py-1 text-white rounded-md mx-1 text-sm font-bold'>Delete</button>
          </div>
          </div>
           })}
        </div>
